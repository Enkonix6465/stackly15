...
<<<<<<< HEAD
  ...
  return (
    <div className="dashboard">
      {/* Header with logo left, title centered, actions right */}
      <div className="dashboard-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, position: 'relative' }}>
        <div style={{ flex: '0 0 auto', display: 'flex', alignItems: 'center' }}>
          <Link to="/home1" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none' }}>
            <img src={logo} alt="STACKLY" style={{ height: 44, width: 'auto', cursor: 'pointer', verticalAlign: 'middle' }} />
          </Link>
        </div>
...
<<<<<<< HEAD
          ...
          >
            ...
            <span style={{ whiteSpace: 'nowrap' }}>{t.createEvent}</span>
          </button>
          <button
            className="btn danger cta-btn"
            onClick={() => navigate("/login")}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 18px', fontSize: 16, minWidth: 0 }}
          >
...
<<<<<<< HEAD
      ...
      <div className="recent-activities-box" style={{ background: 'var(--card-bg)', borderRadius: 12, boxShadow: 'var(--shadow)', padding: 24, margin: '32px 0', maxWidth: 600, position: 'relative' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{ color: 'var(--primary-color)', background: 'none', border: 'none', fontWeight: 600, cursor: 'pointer', fontSize: 15, padding: 0 }}
              onClick={() => setShowAllDropdown((v) => !v)}
            >
              View All ▼
            </button>
            {showAllDropdown && (
              <div style={{ position: 'absolute', right: 0, top: 30, background: 'var(--card-bg)', border: '1px solid #eee', borderRadius: 8, boxShadow: 'var(--shadow)', zIndex: 10, minWidth: 140 }}>
                <button style={{ display: 'block', width: '100%', background: 'none', border: 'none', padding: '10px 16px', textAlign: 'left', cursor: 'pointer', fontWeight: viewSection === 'activities' ? 700 : 400 }} onClick={() => { setViewSection('activities'); setShowAllDropdown(false); }}>Activities</button>
                <button style={{ display: 'block', width: '100%', background: 'none', border: 'none', padding: '10px 16px', textAlign: 'left', cursor: 'pointer', fontWeight: viewSection === 'events' ? 700 : 400 }} onClick={() => { setViewSection('events'); setShowAllDropdown(false); }}>Events</button>
                <button style={{ display: 'block', width: '100%', background: 'none', border: 'none', padding: '10px 16px', textAlign: 'left', cursor: 'pointer', fontWeight: viewSection === 'content' ? 700 : 400 }} onClick={() => { setViewSection('content'); setShowAllDropdown(false); }}>Content</button>
                <button style={{ display: 'block', width: '100%', background: 'none', border: 'none', padding: '10px 16px', textAlign: 'left', cursor: 'pointer', fontWeight: viewSection === 'analytics' ? 700 : 400 }} onClick={() => { setViewSection('analytics'); setShowAllDropdown(false); }}>Analytics</button>
              </div>
            )}
        {viewSection === 'analytics' && (
          <React.Fragment>
            ...
            <div style={{ padding: '10px 0', fontSize: 15 }}>
              <div style={{ marginBottom: 16 }}><strong>Sales Channels:</strong></div>
              <div style={{ maxWidth: 300, margin: '0 auto 20px auto' }}>
                <Pie data={salesChannelData} options={{ plugins: { legend: { display: true, position: 'bottom' } } }} />
              </div>
              <div style={{ margin: '20px 0 8px 0' }}><strong>Top Events:</strong></div>
              <div style={{ maxWidth: 300, margin: '0 auto' }}>
                <Bar data={topEventsData} options={{ plugins: { legend: { display: false } }, scales: { x: { title: { display: true, text: 'Event' } }, y: { title: { display: true, text: 'Registrations' }, beginAtZero: true } } }} />
              </div>
            </div>
          </React.Fragment>
        )}
        ...
        {/* Section content */}
        {viewSection === 'activities' && (
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {recentActivities.map((activity, idx) => (
              <li key={activity.id} style={{ padding: '10px 0', borderBottom: idx < recentActivities.length - 1 ? '1px solid #f0f0f0' : 'none', fontSize: 15 }}>
                <span style={{ fontWeight: 600 }}>{activity.user}</span> {activity.action} <span style={{ color: '#888', fontSize: 13 }}>{activity.time}</span>
              </li>
            ))}
          </ul>
        )}
        {viewSection === 'events' && (
          <React.Fragment>
            ...
            {events.map((event, idx) => (
              <li key={event.name + event.date} style={{ padding: '10px 0', borderBottom: idx < events.length - 1 ? '1px solid #f0f0f0' : 'none', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <span style={{ fontWeight: 600 }}>{event.name}</span> — {event.date}, {event.location} | Regs: {event.regs} | Price: ${event.price}
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn cta-edit" style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, background: '#222', color: '#fff', border: 'none', cursor: 'pointer', padding: '6px 14px', borderRadius: 6, lineHeight: 1.2, fontSize: 16 }} onClick={() => handleEdit(idx)}>
                    <FaEdit style={{ marginRight: 2 }} /> Edit
                  </button>
                  <button className="btn danger cta-delete" style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, background: '#e53935', color: '#fff', border: 'none', cursor: 'pointer', padding: '6px 14px', borderRadius: 6, lineHeight: 1.2, fontSize: 16 }} onClick={() => handleDelete(idx)}>
                    <FaTrash style={{ marginRight: 2 }} /> Delete
                  </button>
                </div>
              </li>
            ))}
        ...
        {viewSection === 'content' && (
          <React.Fragment>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {content.map((c, idx) => (
              <li key={c.title} style={{ padding: '10px 0', borderBottom: idx < content.length - 1 ? '1px solid #f0f0f0' : 'none', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <span style={{ fontWeight: 600 }}>{c.title}</span> — {c.desc}
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="btn cta-edit" style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, background: '#222', color: '#fff', border: 'none', cursor: 'pointer', padding: '6px 14px', borderRadius: 6, lineHeight: 1.2, fontSize: 16 }}>
                    <FaEdit style={{ marginRight: 2 }} /> Edit
                  </button>
                  <button className="btn danger cta-delete" style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, background: '#e53935', color: '#fff', border: 'none', cursor: 'pointer', padding: '6px 14px', borderRadius: 6, lineHeight: 1.2, fontSize: 16 }}>
                    <FaTrash style={{ marginRight: 2 }} /> Delete
                  </button>
                </div>
              </li>
